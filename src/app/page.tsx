              ...
              <div className="lg:col-span-5 rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm space-y-6">
                ...
                {addMode === "single" ? (
                  <form onSubmit={handleAddSingleSmtp} className="space-y-4">
                    <div>
                      <label className="block text-xs font-semibold text-zinc-300 mb-1">Sender Email Address</label>
                      <input
                        type="email"
                        value={newSmtpEmail}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-zinc-300 mb-1">Gmail App Password (16-digits)</label>
                      <input
                        type="password"
                        value={newSmtpPass}
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-zinc-300 mb-1">Daily Limit per Account</label>
                      <input
                        type="number"
                        value={newSmtpLimit}
                              ...
                              <td className="p-3">
                                <div className="space-y-1">
                                  <div className="flex items-center justify-between text-[11px]">
                                    <span className="font-semibold text-zinc-300">{acc.sentToday} / {acc.dailyLimit}</span>
          ...
          <div className="space-y-8 max-w-7xl mx-auto">
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
              ...
              <div className="lg:col-span-7 rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm space-y-6">
                ...
                <form onSubmit={handleSendCampaign} className="space-y-4">
                  ...
                  <div>
                    <label className="block text-xs font-semibold text-zinc-300 mb-1">Email Subject Line</label>
                    <input
                      type="text"
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="block text-xs font-semibold text-zinc-300 mb-1">From Name</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-xs font-semibold text-zinc-300 mb-1">From Email (Multi-Account Rotation)</label>
                      <input
                        type="email"
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      <label className="block text-xs font-semibold text-zinc-300">HTML Template Content</label>
                      <button
                        type="button"
                        onClick={() => setShowAiModal(true)}
              ...
              <div className="lg:col-span-5 space-y-6">
                <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm flex flex-col">
                  ...
                  <div className="rounded-xl bg-zinc-950 p-3 border border-zinc-800 text-xs space-y-1 mb-4 text-zinc-300">
                    <p><strong className="text-zinc-400">From:</strong> {campaignForm.fromName} &lt;{campaignForm.fromEmail}&gt;</p>
                    <p><strong className="text-zinc-400">To:</strong> Jithendra Varma &lt;jithendravarma.l@gmail.com&gt;</p>
                    <p><strong className="text-zinc-400">Subject:</strong> {campaignForm.subject}</p>
                  </div>

                  <div
                    className={`mx-auto transition-all duration-300 border border-zinc-800 rounded-xl bg-white shadow-inner overflow-hidden ${
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm space-y-4">
              ...
              {campaignsList.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {campaignsList.map((c: any) => (
                        <tr key={c.id} className="hover:bg-zinc-800/60">
                          ...
                          <td className="p-3 text-zinc-500">{c.sentAt ? new Date(c.sentAt).toLocaleString() : "Draft"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-xs text-zinc-500">No campaigns created yet.</p>
              )}
            </div>
          </div>
        )}
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-12">
              <div className="lg:col-span-6 rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm space-y-4">
                ...
                <form onSubmit={handleRegisterDomain} className="space-y-4">
                  <div>
                    <label className="block text-xs font-semibold text-zinc-300 mb-1">Domain Name</label>
                    <input
                      type="text"
                      value={inputDomain}
                ...
                {registeredDomains.length > 0 ? (
                  <div className="space-y-4 flex-1">
                    {registeredDomains.map((d: any) => (
                      <div key={d.id} className="rounded-xl border border-zinc-800 bg-zinc-950 p-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center space-x-2">
          ...
          <div className="space-y-6 max-w-7xl mx-auto">
            ...
            <div className="rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm">
              ...
              {analytics?.recentLogs?.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {analytics.recentLogs.map((log: any) => (
                        <tr key={log.id} className="hover:bg-zinc-800/60">
                          ...
                          <td className="p-3 text-zinc-500">{new Date(log.deliveredAt).toLocaleString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-xs text-zinc-500 py-4">No email logs found yet for tenant {workspaceId}. Send a campaign to populate analytics!</p>
              )}
            </div>
          </div>
        )}
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-12 max-w-7xl mx-auto">
            ...
            <div className="lg:col-span-8 rounded-2xl border border-zinc-800 bg-zinc-900 p-6 shadow-sm">
              ...
              {subscribersData?.subscribers?.length > 0 ? (
                    ...
                    <tbody className="divide-y divide-zinc-800">
                      {subscribersData.subscribers.map((sub: any) => (
                        <tr key={sub.id} className="hover:bg-zinc-800/60">
                          ...
                          <td className="p-3 text-zinc-500">{new Date(sub.createdAt).toLocaleDateString()}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
                ...
                <p className="text-xs text-zinc-500 py-4">No subscribers found for tenant {workspaceId}.</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="w-full max-w-lg rounded-2xl bg-zinc-900 border border-zinc-800 p-6 shadow-2xl space-y-6">
            ...
            <form onSubmit={handleGenerateAiTemplate} className="space-y-4">
              <div>
                <label className="block text-xs font-semibold text-zinc-300 mb-1">What do you want this email to be about?</label>
                <textarea
                  rows={4}
                  value={aiPrompt}
              ...
              <div>
                <label className="block text-xs font-semibold text-zinc-300 mb-1">Email Tone</label>
                <select
                  value={aiTone}
                ...
                >
                  ...
                  <option value="Professional" className="bg-zinc-900 text-white">💼 Professional & Corporate</option>
                  <option value="Persuasive" className="bg-zinc-900 text-white">🎯 Persuasive Call-to-Action</option>
                  <option value="Casual" className="bg-zinc-900 text-white">💬 Casual & Personal</option>
                </select>
              </div>
              ...
              <div className="flex items-center justify-end space-x-3 pt-2">
                <button
                  type="button"
                  onClick={() => setShowAiModal(false)}
                  className="rounded-xl border border-zinc-800 px-4 py-2 text-xs font-bold text-zinc-400 hover:bg-zinc-800 hover:text-white"
                >
                  Cancel
                </button>
                <button
